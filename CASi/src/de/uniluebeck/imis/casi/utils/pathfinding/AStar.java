...
/**
 * This class implements the behavior of the A<sup>*</sup> algorithm. Subclasses
 * can provide a customize behavior.
 * 
 * @author Tobias Mende
 * 
 * @param <T>
 *            the type of nodes that are handled by this class
 */
public abstract class AStar<T> {
	 ...
	 * @param node
	 *            the node to check
	 * @return <code>true</code> if the node is destination, <code>false</code>
	 *         otherwise.
	 */
	protected abstract boolean isDestination(T node);

	/**
	 * Calculates the costs for getting from the node <code>from</code> to the
	 * node <code>to</code>
	 * 
	 * @param from
	 *            the previous node
	...
	/**
	 * Calculates an optimistic heuristic which should never be bigger than the
	 * real costs. In graphic cases linear distance is an acceptable heuristic.
	 * 
	 * @param from
	 *            the previous node
	 * @param to
	 ...
	 * 
	 * @param node
	 *            the node to get successors for
	...
	/**
	 * Calculates <code>f(x) = g(x) + h(x)</code> which represents the minimum
	 * total costs.
	 * 
	 * @param path
	 *            the path we use
	...
	/**
	 * Computes the shortest path to the destination from the <code>start</code>
	 * node
	 * 
	 * @param start
	 *            the start node

...
/**
 * This class provides a solver using A<sup>*</sup> which is able to find a path in a given room.
 * @author Tobias Mende
 *
 */
public class InRoomPathSolver extends AStar<Point> {
	 ...
	 * @param room the room to search path in
	 * @param destination the destination point
	 * @throws IllegalArgumentException if the destination is not in the room.
	 */
	public InRoomPathSolver(Room room, Point destination) throws IllegalArgumentException{
		this.room = room.getShapeRepresentation();
		this.destination = destination;
		if(!this.room.contains(destination)) {
			throw new IllegalArgumentException("The destination is not in the room. Can't get path for it.");
		}
	}
	@Override
